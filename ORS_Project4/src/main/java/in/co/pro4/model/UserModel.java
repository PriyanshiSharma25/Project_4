...
public class UserModel {
	...
	public static long add(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		if (beanExists != null) {
			...
			throw new DuplicateRecordException("Enter new login Id. This LogIn ID already Exists..!");
		}
		try {
			conn = JDBCDataSource.getConnection();
			pk = nextPK();
...
try {conn.rollback();} catch (Exception e1) {throw new ApplicationException("Exception : add rollback=" + e1.getMessage());}
	...
	public void update(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			conn.setAutoCommit(false);
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE ST_USER SET FIRST_NAME=?, LAST_NAME=?, LOGIN=?, DOB=?, MOBILE_NO=?, ROLE_ID=?, UNSUCCESSEFUL_LOGIN=?, GENDER=?, LAST_LOGIN=?, USER_LOCK=?, REGISTERED_IP=?, LAST_LOGIN_IP=?, CREATED_BY=?, MODIFIED_BY=?, CREATED_DATETIME=?, MODIFIED_DATETIME=? WHERE ID=?");
